...
export function VendorListScreen() {
  ...
  const canCreate = useAuthStore((s) => s.hasPermission('vendor:create'));
  ...
  const {
    ...
    queryKey: ['vendors', 'list', { search: debouncedSearch }],
    queryFn: async () => {
      const res = await apiService.get<{ data: Vendor[] }>(API_ENDPOINTS.VENDORS.LIST, {
        params: {
  ...
  const sections = useMemo(() => {
    ...
    (vendors ?? []).forEach((v) => {
      const letter = (v.displayName?.[0] ?? '#').toUpperCase();
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await queryClient.invalidateQueries({ queryKey: ['vendors', 'list'] });
    setRefreshing(false);
  }, [queryClient]);
      ...
      if (Platform.OS === 'ios') {
        ActionSheetIOS.showActionSheetWithOptions(
          ...
          (idx) => {
            ...
            if (chosen === 'Call') Linking.openURL(`tel:${vendor.phone}`);
            if (chosen === 'Email') Linking.openURL(`mailto:${vendor.email}`);
            if (chosen === 'Edit') navigation.navigate('VendorForm', { id: vendor.id });
          },
        );
      } else {
        Alert.alert(vendor.displayName, undefined, [
          ...(hasPhone
            ? [{ text: 'Call', onPress: () => Linking.openURL(`tel:${vendor.phone}`) }]
            : []),
          ...(hasEmail
            ? [{ text: 'Email', onPress: () => Linking.openURL(`mailto:${vendor.email}`) }]
            : []),
          { text: 'Edit', onPress: () => navigation.navigate('VendorForm', { id: vendor.id }) },
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: BG }} edges={['top']}>
      ...
      >
        <View>
          ...
          <Text style={{ fontSize: 14, color: MUTED, marginTop: 2 }}>
            {isLoading ? '…' : `${vendors?.length ?? 0} vendors`}
          </Text>
        </View>
        {canCreate && (
          <TouchableOpacity
      ...
      <View style={{ paddingHorizontal: 16, paddingBottom: 12 }}>
        ...
        >
          ...
          <TextInput
            ...
            placeholder="Search names, emails, or phone..."
      ...
      <View style={{ flex: 1 }}>
        ...
        {!isLoading && !isError && (vendors ?? []).length === 0 && (
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 32 }}>
            <MaterialCommunityIcons name="store-outline" size={64} color={OUTLINE_V} />
            <Text
              style={{
                fontSize: 18,
                fontWeight: '600',
            ...
            >
              {debouncedSearch ? 'No results found' : 'No vendors yet'}
            </Text>
            <Text style={{ fontSize: 14, color: MUTED, marginTop: 6, textAlign: 'center' }}>
              {debouncedSearch
                ? 'Try a different search term'
                : 'Add your first vendor to start managing bills'}
            </Text>
